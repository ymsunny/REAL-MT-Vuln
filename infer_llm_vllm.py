        ...
        with torch.no_grad():
            for _ in range(model_config.max_tokens):
                ...
                try:
                    ...
                    self.logger.warning(f"模型前向传播异常: {str(e)}")
...
def process_batch_translation(inputs, translation_task, model_config, src, tgt, prompt_num, prompt_dict,
    ...
    """处理批量翻译"""
    results = []
    batch_size = 40  # 设置批处理大小
    
    # Initialize the tokenizer
    tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen2.5-7B-Instruct") #Qwen/Qwen3-8B-Base
    ...
    # 按批次处理数据
    for batch_start in tqdm(range(0, len(inputs), batch_size), desc="批次处理进度"):
        batch_end = min(batch_start + batch_size, len(inputs))
        batch_items = inputs[batch_start:batch_end]
        
        # 存储每个批次的所有prompts
        all_prompts = []
        prompt_mapping = []  # 记录每个prompt对应的item索引和mode
        ...
        try:
            # 批量生成
            logger.info(f"处理批次 {batch_start//batch_size + 1}, prompts数量: {len(all_prompts)}")
            # Generate outputs
            ...
            for i, output in enumerate(outputs):
                ...
                if output and hasattr(output.outputs[0], 'text'):
                    mapping = prompt_mapping[i]

                    print(f"output.outputs[0].text is {output.outputs[0].text}")
                    
                    # 提取思考内容和剩余文本
                    think_content, remaining_text = extract_think_content(output.outputs[0].text)
                    
                    print(f"i is {i}, remaining_text is {remaining_text}")
                    # 处理翻译结果
                    translation_text, _ = extract_json_response1(remaining_text, logger)

                    print(f"translation_text is {translation_text}")
                    
                    # 更新对应的结果条目
                    if think_content:
                        if "think_response" not in mapping['result_entry']["translation"]:
                            mapping['result_entry']["translation"]["think_response"] = {}
                        mapping['result_entry']["translation"]["think_response"][mapping['mode']] = think_content
                    
                    mapping['result_entry']["translation"][mapping['mode']] = translation_text
                else:
                    logger.warning(f"输出 {i} 无效或为空")
            
            # 收集这个批次的结果
            batch_results = [None] * len(batch_items)
            for mapping in prompt_mapping:
                batch_results[mapping['batch_idx']] = mapping['result_entry']
            
            # 将批次结果添加到总结果中
            results.extend([r for r in batch_results if r is not None])
            
            # 定期保存中间结果
           #save_interim_results(results, output_file, batch_start + batch_size - 1, len(inputs))
            
        except Exception as e:
            logger.error(f"处理批次 {batch_start//batch_size + 1} 时出错: {str(e)}", exc_info=True)
            # 将错误信息添加到结果中
            for mapping in prompt_mapping:
                mapping['result_entry']["translation"][mapping['mode']] = f"ERROR: {str(e)}"
...
if __name__ == '__main__':
    ...
    if args.input_file:
        ...
        # 构建输出文件名
        output_file_basename = os.path.splitext(input_file_basename)[0]
        output_file_name = f"{output_file_basename}_{args.meaning_lang}_{model_name_short}_no_thinking_{args.prompt_num}.json"
